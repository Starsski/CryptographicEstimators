...
class RankSDKeyAttack(RQCAlgorithm):
    def __init__(self, problem: RQCProblem, **kwargs):
        """Construct an instance of RankSDKeyAttack for RQC.

        Estimates the complexity of a key-recovery attack on RQC by reducing it
        to a Rank Syndrome Decoding (RSD) instance.  The public key of RQC is a
        quasi-cyclic code of length 2n and dimension n over F_{q^m}; recovering
        the secret key of rank weight w reduces to solving
